...
# lashings are continuous rope curves rasterised directly onto the grid.
#
...
#  * neighbouring logs slightly overlap to guarantee lattice connectivity;
#    this is a voxel allowance, not physical compression of solid wood;
...
def _build_raft(spec: dict, rng, voxel_m: float, steps: _Steps) -> VoxelGrid:
    ...
    radii = r0 * (1.0 + 0.12 * (rng.random(n_logs) * 2.0 - 1.0))
    ...
    # Match the authored beam; overlap is only a lattice connection allowance,
    # not a claim that rope can compress solid timber.
    width = ys[-1] - ys[0] + radii[0] + radii[-1]
    ys *= B / width
    radii *= B / width
    ...
    for i in range(n_logs):
        ...
        # Continuous taper, slight bow and longitudinal bark flutes. No noise
        # holes or independent voxel jitter: the timber remains solid.
        cy = ys[i] + 0.35 * voxel_m * np.sin(math.pi * X / L + phase)
        theta = np.arctan2(Z - zc[i], Y - cy)
        radial = np.sqrt((Y - cy) ** 2 + (Z - zc[i]) ** 2)
        radius = radii[i] * (1 + 0.055 * np.cos(math.pi * X / L + phase))
        radius = radius + 0.22 * voxel_m * np.sin(9 * theta + phase + X * 1.4)
        # Uneven axe-cut ends, still exposing a broad readable wood face.
        chip = 0.65 * voxel_m * np.sin(3 * theta + phase)
        start = stagger[i, 0] + chip
        end = L + stagger[i, 1] + chip
        mask = (radial <= radius) & (X >= start) & (X <= end)
        log_masks.append(mask)
        log_fields.append((radial, radius, theta, start, end, phase))
    ...
    if chosen:
        ...
        for i in sorted(chosen):
            radial, radius, theta, start, end, phase = log_fields[i]
            # Long worn patches, rather than entire logs painted grey.
            pick |= (log_masks[i] & (radial > radius - voxel_m)
                     & (np.sin(3 * theta + phase + 0.7 * X) > 0.15))
        took = sum(counts[i] for i in chosen) / max(sum(counts), 1)
    ...
    def draw_wood():
        for i, mask in enumerate(log_masks):
            radial, radius, theta, start, end, phase = log_fields[i]
            ends = (X < start + voxel_m) | (X > end - voxel_m)
            peeled = ((np.sin(4 * theta + phase + 0.8 * X) > 0.88)
                      & (np.sin(1.7 * X + phase) > -0.3)
                      & (Z > zc[i]))
            core = radial < radius - 1.1 * voxel_m
            _paint(grid, mask & (core | ends | peeled), mat_pole)
            # Two irregular growth bands and a radial drying check on the ends.
            grain = ((np.abs(radial - radii[i] * 0.42) < voxel_m * 0.32)
                     | (np.abs(radial - radii[i] * 0.73) < voxel_m * 0.25))
            check = (np.abs(np.sin(theta - phase)) < 0.09) & (radial > radius * 0.5)
            _paint(grid, mask & ends & (grain | check), mat_weathered)
            _paint(grid, mask & ends & (radial > radius - voxel_m * 0.65), mat_log)
    steps.run("exposed wood and end grain", grid, draw_wood,
              note="axe-cut ends, growth bands, drying checks and peeled bark")
    ...
    # --- lashings ------------------------------------------------------------
    # Three rope turns at each crossing. The upper arc rides over the pole;
    # the lower arc follows the log. Wood stays visible between the turns.
    if lash_x:
        def draw_lash():
            for xk in lash_x:
                for i in range(n_logs):
                    # Three individually resolved turns around the log and
                    # over the crosspiece, with a small axial skew. Curves are
                    # rasterised directly onto the cubic lattice.
                    radial, radius, theta, start, end, phase = log_fields[i]
                    local_r = radii[i] * (1 + 0.055 * math.cos(math.pi * xk / L + phase))
                    cy = ys[i] + 0.35 * voxel_m * math.sin(math.pi * xk / L + phase)
                    for turn in (-1, 0, 1):
                        pts = []
                        for angle in np.linspace(0, 2 * math.pi, 65):
                            sn, cs = math.sin(angle), math.cos(angle)
                            y = cy + (local_r + rope_t * 0.3) * cs
                            z = zc[i] + (local_r + rope_t * 0.3) * sn
                            # Lift the upper arc onto the pole; bury the
                            # underside slightly so every turn touches timber.
                            z += max(0.0, sn) ** 4 * (z_pole + r_pole - zc[i] - local_r)
                            x = xk + turn * 2.0 * voxel_m + 0.35 * voxel_m * cs
                            pts.append(_to_vox((x, y, z), lo, voxel_m))
                        _polyline(grid, pts, rope_t / voxel_m, mat_rope)
                    # A tightening knot across the turns and a short loose tail.
                    knot_z = z_pole + r_pole + 0.35 * voxel_m
                    pts = [_to_vox(p, lo, voxel_m) for p in (
                        (xk - 2.5 * voxel_m, cy, knot_z),
                        (xk + 2.5 * voxel_m, cy + voxel_m, knot_z),
                        (xk + 4 * voxel_m, cy + 1.5 * voxel_m, z_top))]
                    _polyline(grid, pts, rope_t / voxel_m, mat_rope)
        steps.run("lashings", grid, draw_lash,
                  note=f"{len(lash_x)} stations x {n_logs} straps, "
                       f"3 turns and knot each, {2 * rope_t * 100:.1f} cm rope")
    else:
