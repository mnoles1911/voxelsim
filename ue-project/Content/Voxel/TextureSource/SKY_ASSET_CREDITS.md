...
notices live). Since 2026-08-09 the STAR MAP line applies only to builds that
import the NASA EXR -- the default star map is procedural and contains no NASA
data (see the next section). The MOON lines always apply.
...
## The star map in use is now PROCEDURAL, not NASA's

Since 2026-08-09, `T_SkyStarmap` is imported from
`tools/sky-assets/starfield_procedural_8k.png` — 8192x4096, 16-bit, generated by
`ue-project/Tools/gen_starfield_texture.py` (system Python, numpy + Pillow, fixed
seed 20260809). It contains no NASA data and needs no attribution.

**Why.** The owner's report was "the night skybox is too grainy", and the NASA
map's grain is measurable rather than a matter of taste: in a starless patch its
texel-scale deviation is **3.5x its own local mean**, its stars carry no
point-spread function (100609 single-texel local maxima above 0.05), and at
4096 texels of right ascension one texel covers ~2.5 screen pixels at 2K/90°, so
that noise is magnified rather than minified. Night renders at a fixed
deep-night exposure, so nothing tone-maps it away. The generator's docstring
carries the full measurement and the design that answers it.

The two maps are **interchangeable**: both carry the same UV convention, both
land at `/Game/Voxel/T_SkyStarmap`, and `import_sky_textures.py` picks the
procedural PNG when it exists and the NASA EXR when it does not.

```powershell
python .\ue-project\Tools\gen_starfield_texture.py     # ~30 s, writes the PNG
# then the same import command as below

$env:VOXEL_STARMAP = 'nasa'    # force the old 4k NASA map back (the A/B control)
```

Side-by-side crops of the two, at matched sky coordinates and matched exposure,
are in `bake-out/starfield/` with their conditions.

**Attribution note.** The required NASA credit below still applies to
`T_MoonColor` and `T_MoonDisplacement`, which are unchanged. It applies to the
star map only in builds that import the NASA EXR.
