...
// ---------------------------------------------------------------------------
DECLARE_GPU_STAT_NAMED(VoxelStreamMeshJob, TEXT("VoxelStreamMeshJob"));
DECLARE_GPU_STAT_NAMED(VoxelStreamQuadIO, TEXT("VoxelStreamQuadIO"));
DECLARE_GPU_STAT_NAMED(VoxelStreamBrickStack, TEXT("VoxelStreamBrickStack"));
...
namespace
{
	...
	{
		// NESTED, AND THE COLUMN IS STILL A CLEAN SPLIT. The CSV value is the
		// stat's EXCLUSIVE busy+wait (GPUProfiler.cpp:1066), charged only while
		// this stat is the INNERMOST open one (:1548-1552), so the enclosing
		// VoxelStreamMeshJob reports itself MINUS this one and every GPU/ column
		// still sums to the queue total. Read the enclosing name as "the rest
		// of", never as the whole.
		RDG_EVENT_SCOPE_STAT(GraphBuilder, VoxelStreamBrickStack, "VoxelStreamBrickStack");
...
void FVoxelGpuMeshJobManager::DispatchBatch(TArray<FJobPtr>&& Batch)
{
		 ...
		 bVerifyPackArmed](FRHICommandListImmediate& RHICmdList)
	{
		...
		RHI_BREADCRUMB_EVENT_STAT(RHICmdList, VoxelStreamMeshJob, "VoxelStreamMeshJob");
...
void FVoxelGpuMeshJobManager::DispatchQuadFetch(TArray<FJobPtr>&& Batch)
{
		...
		[Jobs = MoveTemp(Batch)](FRHICommandListImmediate& RHICmdList)
	{
		...
		// the same GPU/<name> CSV column, and outlives the graph by construction
		// (declared before it, destroyed after it).
		RHI_BREADCRUMB_EVENT_STAT(RHICmdList, VoxelStreamQuadIO, "VoxelStreamQuadIO");
		FRDGBuilder GraphBuilder(RHICmdList);
