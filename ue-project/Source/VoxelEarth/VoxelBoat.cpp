...
void AVoxelBoat::TickBuoyancy(float DeltaSeconds)
{
	...
	for (int32 I = 0; I < NumProbes; ++I)
	{
		...
		if (bUsable)
		{
			...
			const double WaterDepth = FMath::Max(0.0, S.SurfaceZUU - S.GroundZUU);
		...
		if (bSubmerged)
		{
			...
			{
				DrawDebugLine(World, P, FVector(P.X, P.Y, S.SurfaceZUU), FColor::Cyan, false, 0.f, 0,
				              1.5f);
			}
		}
...
void AVoxelBoat::TickWake(float DeltaSeconds)
{
	...
	if (bHaveLastWake && bAnyWet && SpeedXY > WakeMinSpeedUU)
	{
		...
		const float Frac = float(VoxelBoatLocal::ImpactFraction(SpeedXY));
