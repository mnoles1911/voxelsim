...
def _write_all(out):
    """Write the three PNGs into `out`. Returns lines describing what was made."""
    os.makedirs(out, exist_ok=True)
    lines = []

    write_palette(os.path.join(out, "T_VoxelPalette.png"))
    # The width is PALETTE_WIDTH, not a literal: this line said "16x1" while
    # emitting 47x1, because the table grew from the sixteen terrain materials
    # to the whole enum and the format string did not. A log that states a
    # dimension it did not measure is worse than one that omits it.
    #
    # NOTHING SAMPLES THIS TEXTURE. terrain_material_common.py's PALETTE_TEXTURE
    # is a dead constant and its header says why -- the id came through
    # VertexColor.R as ~6-8 where the CPU wrote 4, so categorical thresholds all
    # read 0, and the pooled path does the compare in the shader instead. It is
    # still generated and imported, so it is still kept honest here.
    lines.append("T_VoxelPalette.png   %dx1   materials=%d biome-tinted-runs=%s "
                 "(currently sampled by nothing)"
                 % (PALETTE_WIDTH, len(PALETTE), biome_tinted_runs()))

    b = write_biome_lut(os.path.join(out, "T_VoxelBiomeLUT.png"))
    lines.append("T_VoxelBiomeLUT.png  %dx%d  temp u8 %d..%d, precip u8 %d..%d"
                 % (b.shape[0], b.shape[1], TEMP_U8_LO, TEMP_U8_HI,
                    PRECIP_U8_LO, PRECIP_U8_HI))

    d = write_detail(os.path.join(out, "T_VoxelDetail.png"))
    lines.append("T_VoxelDetail.png    %dx%d  tiling fBm, mean=%.3f"
                 % (d.shape[0], d.shape[1], d.mean() / 255.0))
    return lines


NAMES = ("T_VoxelPalette.png", "T_VoxelBiomeLUT.png", "T_VoxelDetail.png")


def check(out):
    """Regenerate into a temp directory and compare PIXELS with what is committed.

    WHY THIS EXISTS. These PNGs are the only copy of the palette that Unreal
    reads for the vista, and they are regenerated by hand. When ADR-0009 made
    write_biome_lut paint from the palette instead of from six authored anchors,
    the function changed and the committed PNG did not -- so the clipmap kept the
    old colours while the near field moved to the new ones, and the seam that
    change exists to close would still have been there after a full editor trip.
    Nothing said so; the file just sat at its old bytes. VoxelClimateProbe.h
    already carries a static_assert whose message says the LUT "must be
    regenerated in the same commit". The rule was written down and not followed,
    which is what a check is for.

    PIXELS, NOT BYTES, and the distinction is the whole design. The generator is
    deterministic -- SEED is fixed and the biome grid comes from vxc_biomelut,
    which is integer-only -- so two runs on one box are byte-identical and a byte
    compare looks correct. It is not: PNG is a container, and its encoder writes
    different bytes for the same image across Pillow versions. Measured here,
    T_VoxelDetail.png differs from the committed file in every byte and in ZERO
    pixels. A byte compare would call that stale on any box but the one that last
    generated it -- which is exactly the failure that made asset-forge's palette
    drift check useless for months, and it would be reintroduced here with the
    same shape.

    So: decode both, compare dimensions and samples. What the engine imports is
    the pixels; the container is not content.
    """
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        stale, missing = [], []
        for name in NAMES:
            have = os.path.join(out, name)
            if not os.path.exists(have):
                missing.append((name, "missing"))
                continue
            a = np.asarray(Image.open(have).convert("RGB"), dtype=np.int32)
            b = np.asarray(Image.open(os.path.join(tmp, name)).convert("RGB"),
                           dtype=np.int32)
            if a.shape != b.shape:
                stale.append((name, f"{a.shape[1]}x{a.shape[0]} on disk, "
                                    f"{b.shape[1]}x{b.shape[0]} generated"))
            elif (a != b).any():
                d = np.abs(a - b)
                n = int((d.max(axis=2) > 0).sum())
                stale.append((name, f"{n} of {a.shape[0] * a.shape[1]} pixels "
                                    f"differ, worst channel delta {int(d.max())}"))

    if not stale and not missing:
        print("terrain textures match the palette and the biome classifier: pass")
        return 0

    for name, why in missing + stale:
        print(f"  STALE: {name} -- {why}")
    print("\n  run python ue-project/Tools/gen_terrain_textures.py and commit the "
          "PNGs.\n"
          "  Tools/import_terrain_textures.py is what brings these into the "
          "editor; a stale\n"
          "  T_VoxelBiomeLUT ships as a 50 km vista that disagrees with the near "
          "field, with\n"
...
def main():
    ...
    ap.add_argument("--check", action="store_true",
                    help="do not write; fail if the committed PNGs have drifted")
    a = ap.parse_args()
