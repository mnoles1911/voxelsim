...
**What this is.** The one description of the water system as it actually stands.
It replaces the plan that used to live at this path (written 2026-08-03, most of
it built or overtaken since; the old text is in git history). The file name is
kept so existing links still resolve.

**Rewritten 2026-08-05** against `main` at `0db6622`, `TERRAIN_VERSION` 8,
`BAKE_VERSION` 14, `kWorldGenVersion` 23, `kWaterCAVersion` 5. Every number
below was read from the code or measured with a probe on the day; where a number
came from somewhere else it says so.

**Read this with two other documents, and only two:**

| document | what it is for |
|---|---|
| `docs/water-deep-dive-brief-2026-08-05.md` | **What is wrong now.** Open defects, ranked by cost to the player, and the seventeen explanations already killed by measurement. Forward-looking. |
| this file | **How it works now.** The stages, the constants, the files, the version rules, and how to measure any of it yourself. |
| `docs/water-handover-2026-08-04.md` | **How it got here.** The session record. Useful for archaeology; several of its numbers have since moved and are corrected in place. |

Everything else water-related is either a measurement record (keep, never
delete) or a superseded plan. §12 says which is which.
...
| file | what it holds |
|---|---|
| `docs/water-deep-dive-brief-2026-08-05.md` | open defects and falsified hypotheses. The forward-looking document |
| this file | the system as built |
...
| file | superseded by |
|---|---|
| `docs/water-production-plan.md` (2026-08-02) | this file (the analysis) and the brief (the work list). Its branch triage in §10 is entirely spent |
| `docs/water-waves-plan-2026-08-04.md` | the brief. Waves A and C0 landed; B2 closed with a negative result |
...
* **"82.5 m³/s, the world's largest river."** It is a **submarine sink at
  ...
  **This claim is still live in the docstring of
  `terrain-service/tools/survey_world_water.py`** (lines ~31–36), which is a code
  file and was deliberately not edited by this documentation pass. It should be
  corrected there.
...
* **"The amplifier buries the river."** +3 mm at the centreline (p50).
