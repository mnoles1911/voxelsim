...
5. **Authored-LOD cache rebaked and the size-cull A/B rerun with passing receipts.** The regenerated M_VoxelDetailAsset changed the cache's `material_source_sha256` and two builder identity sources (`VoxelDetailAssetSubsystem.cpp`, `VoxelBakeDetailMeshesCommandlet.cpp`) changed in the merge, so `temperate-authored-lods-full-1` was invalid for this binary. New immutable bake `temperate-authored-lods-full-2` (manifest 603c973e..., builder 66f7f4fd..., 339 models; `asset-forge/out/ecological-placement/detail-authored-lod-bake-2`), fresh-process `Tools/verify-detail-mesh-bake.ps1` passed (`detail-authored-lod-verify-2`), and `analyze_authored_lod_bake.py` shows 0 changed LOD models and the same 4,157,030 last-LOD triangles as `-full-1`. Captures 26 (OFF) and 27 (ON) at the full 256 m ring with the handoff's exact settings both passed the fail-closed receipt; the comparator accepted them. GPU median 42.4->21.6 ms walking and 39.2->20.4 ms sprinting; frame time 43.4->38.5 and 51.9->54.5 ms, game-thread bound in both arms. Evidence and reading: `docs/measurements/size-cull-ab-full256-2026-09-10/`. This is the accepted replacement for the invalidated 24/25 pair; it says nothing about residency, popping or the 48 m ring.
6. **R0 diagnostic and retirement pilot staged on branches, unbuilt.** `claude/r0-entry-profile-rebase-2026-09-10`: the R0 patch rebased (hand-placed Evaluations counter at AdmitCandidateEvaluate entry) plus a new `analyze_r0_entry_profile.py` with six tests. `claude/detail-retirement-pilot-2026-09-10`: the retirement patch applied cleanly. Neither is built or run at this point in the record; see later entries.

7. **R0 entry diagnostic built, engaged and measured.** On branch `claude/r0-entry-profile-rebase-2026-09-10` (coherent build `.scratch/r0-build-1`), captures 28 (flags on) and 29 (flags off) both passed receipts with identical module hashes. Engagement proven by the log's five-second lines and the `VoxelStream/R0Entry*` CSV series. Within the measurement window, 17 R0 entries cost 2.14 s of game thread and 97% of it is `Resolve`, the exact-asset resolve per cold XY memo fill (484 fills, ~4.4 ms each); memo, Z predicates, sky and nearest are negligible; nesting held on every frame. Diagnostic overhead when disabled is within noise. The candidate optimisation is now a measured target, not a guess: memoise the footprint resolve across Z chunks while preserving residency epochs and request/error semantics, then A/B it. Evidence: `docs/measurements/r0-entry-profile-2026-09-10/`.

8. **Retirement pilot built and run once; it engaged but retired nothing.** Branch `claude/detail-retirement-pilot-2026-09-10` (stacked on the R0 branch), coherent build, third bake `temperate-authored-lods-full-3` verified with identical geometry. The three staged regressions and five subsystem tests pass 8/8. The hill revisit route ran for the first time OFF (8) and ON (9): 18/18 arrivals both, navigation unchanged, frame medians identical. ON telemetry: 30,623 probes, 0 keys retired, 1,282 HISM deferrals, barrier never blocked. Cause: `ClearInstances` leaves the tree out of date and production never rebuilds a hidden empty tree, so no candidate ever becomes eligible (the async test passed only because the test rebuilds the tree itself). Fix applied (request the empty rebuild once and defer) and rerun as routes 10/11 on a fourth verified bake: 8/8 automation, navigation and frame medians unchanged, and the ON arm now retires (13 keys, 13 objects unrooted, 39 rebuilds requested and cleared). Effect size is small by construction on a continuous 48 m route; the 256 m unload/revisit case and memory accounting are still unmeasured. Evidence: `docs/measurements/detail-retirement-pilot-2026-09-10/`.

9. **Predictive resolve prewarm re-tested with receipts: the R0 target moves.** Same binary and cache, only `-VoxelPredictiveAssetResolve` differs (captures 30 OFF / 31 ON, both receipts passed). Engagement proven: 6,285 warm resolves launched, 6,232 landed; admission cold misses per window 410 -> 6. R0 Resolve total 2,046 -> 559 ms (-73%), Entry p95 254 -> 149 ms. Walking p95 frame time 329 -> 120 ms, sprint median 55.5 -> 48.7 ms; medians elsewhere flat because the GPU floor is unchanged. The earlier 22/23 verdict was confounded by the old LOD payload; this is the first clean pair. **Owner decided 2026-09-10: default ON for ecology worlds** (`-VoxelNoPredictiveAssetResolve` is the off switch; legacy worlds unchanged). Captures 32/33 pin both directions on the shipped default: with no flag at all, 38 predictive windows and 6,310 warm resolves, admission cold misses 173 -> 2 and inline resolve 600 -> 13.6 ms in the final window; with the off switch, every predictive counter is zero. See `docs/measurements/predictive-default-on-2026-09-10/`. Evidence: `docs/measurements/predictive-resolve-ab-2026-09-10/`. Trap recorded: the detail cache identity hashes seven SOURCE files at process start; editing one between arms refuses the cache with identical DLLs (first ON attempt invalidated for exactly that).

10. **Shore survey route run twice: authored line blocked by a beech trunk, detour passes.** `temperate-route-shore-survey-pilot.json` failed 6/7 (route-capture-12), stalled 4.4 m short of the last target. The blocker analyzer over world-capture-15's tree grids names european-beech-0012's trunk on the authored line, with the target centre inside a sapling. `temperate-route-shore-survey-detour.json` keeps all seven targets and adds one via point 4 m west, verified clear in the grids first; route-capture-13 passed 8/8. Evidence and both runs' checkpoint images: `docs/measurements/shore-survey-route-2026-09-10/`. Multiseed shore/hill navigation beyond this fixture is still open.

**State at the end of this session.** PR #254 is merged. PR #255 (https://github.com/mnoles1911/voxelsim/pull/255) carries items 6-10 on `claude/detail-retirement-pilot-2026-09-10` (R0 diagnostic and analyzer, predictive A/B, retirement pilot with fix, tooling, measurements, the switch classification the CI lint asked for). Binaries on the box are that branch's coherent build (`.scratch/retirement-build-2`) and the valid cache for it is `temperate-authored-lods-full-4`; `-full-2` matches PR #254's binary and `-full-3` the pre-fix pilot. Not done, and needing the owner: image verdicts on the checkpoint captures and any popping/lighting acceptance; the decision on `-VoxelPredictiveAssetResolve` as the ecology-world default; the 256 m unload/revisit and memory accounting for retirement; runtime/cooked cache validation. Nothing performance-related beyond the receipts named above is claimed.
