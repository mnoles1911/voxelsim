...
| Character volume | 0.6 × 0.6 × 1.8 m box (6 × 6 × 18 voxels), actor at the box centre. **Eye at 1.67 m** — raised from 1.60 m on 2026-07-28: that was ~7 cm below anthropometric eye height for a 1.80 m person, and measured against the proxy body it put the view 29% up the head mesh (which spans 1.52–1.80 m), i.e. roughly jaw level on the body you can see in third person. 1.67 m lands at 54% up the head. The camera stays at the box centre in X (skull centre, not the face): at the face it would have only 4 UU of clearance before UE's near plane clips through a wall the box is flush against, against 20 UU at centre. |
...
| Character proxy | Blocky voxel-style body (~1.8m, box torso/head/limbs) with basic walk/idle bob — placeholder until M5 per-bone voxel bodies. Visible in TP, hidden in FP. **Revised 2026-07-27:** crouching squashes it vertically by exactly the collision box's half-extent ratio (2/3), so it keeps filling the collision volume in both stances; the limb-swing phase now comes from the movement component, the same phase the first-person head bob uses, so the visible footfalls and the camera dip cannot drift apart. **Pitch bug fixed 2026-07-28:** the body was attached to a root that pitches with the controller, so looking down tipped the character forward and −90° laid it flat on its face — invisible in first person, which is why it survived. It is now counter-rotated to yaw-only in world space (which also fixed the crouch squash, previously leaning with the pitch instead of compressing straight down). **Proportions corrected 2026-07-27:** the body did not match the box it stands in — 24 UU deep against a 60 UU box (40% of the real footprint) while the arms reached ±36 and hung 6 UU *outside* the colliding volume, misreporting the footprint in two directions at once. Torso is now 40 UU deep (67% fill) and the arms sit flush with the box wall at exactly ±30. It still does not reach the corners, which is correct: a 0.6 × 0.6 m footprint is squarer than a human, and the exact bounds are carried by the debug volume below. |
| Player volume debug | **New 2026-07-27** — a scale reference for play-testing, since with 10 cm voxels and a kilometres-wide world there is nothing in frame to judge size against. Draws the live collision AABB as a wireframe (yellow standing, orange crouched), a white marker at the *actual* smoothed eye height, and the voxel cells `IsGroundedAt` is probing — green where solid, red where not, which shows directly why the crouched ledge-stop fires and what the step-up is reasoning about. Walk mode only. **On by default**, gated by `voxel.Debug.PlayerBox` and an F1 overlay row rather than by `voxel.Debug 2` — it is a play aid, not a diagnostic layer, so needing two F3 presses would defeat it. Automatically suppressed in unattended fixture runs so it never lands in a verification capture. |
