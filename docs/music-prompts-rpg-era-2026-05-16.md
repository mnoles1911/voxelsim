<!--
SUPERSEDED 2026-09-09 -- HISTORICAL RECORD. USE docs/music-prompts.md.

This is the 59-cue score plan for the RPG / linear-story era of the game
(Mira-Thal). On 2026-09-09 the owner restarted the OST from first principles:
the game is now an open-ended survival and building game with long solo
sessions and quiet chores, most of this catalogue is too fast and too
event-shaped for that, and the pools it was written for (Town, War, Boss,
Cinematic, Endings) no longer describe the game. What survives from it, and
is restated in the new plan rather than referenced back here: the Suno method
(section 6), the flute constraint, the key-variety rule, and the cues the new
plan explicitly keeps from disk.
-->
